...
locale: zh
lang: zh-CN
excerpt: "概念层与中间层之后，物理层负责把已确定的 assay 转换成可执行的 procedure、transport 与 resources。本文系统梳理 Procedure、Transport、Transferable Resource 及按 I/O 签名分类的 Device 等 Resource 的定义、关系与输出结构。"
...
| engaged | 执行过程中被调用，但不会被一次性消耗 | 仪器、操作员、公共平台、CRO 服务 |
...
- Resource 是 procedure 执行时被消耗或被调用的 Transferable、Device、人与渠道。
